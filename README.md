...
노드를 우클릭하면 다음 작업을 수행할 수 있습니다:

- **작업 시작**: 노드를 `IN_PROGRESS` 상태로 변경
- **완료 처리**: 작업을 `COMPLETED` 상태로 표시
- **상태 변경**: 5가지 상태 중 선택
  - `PENDING`: 선행 작업 대기 중
  - `READY`: 실행 가능 (모든 선행 작업 완료)
  - `IN_PROGRESS`: 현재 진행 중
  - `COMPLETED`: 작업 완료
  - `BLOCKED`: 차단됨
- **노드 복제**: 동일한 속성으로 새 노드 생성
- **노드 삭제**: 노드 및 연결된 모든 엣지 제거

우클릭 메뉴는 현재 노드 상태에 따라 적절한 옵션만 표시됩니다. 예를 들어, 이미 진행 중인 노드는 "작업 시작" 옵션이 표시되지 않습니다.

### 프로젝트 관리

상단 툴바의 **프로젝트 관리** 버튼을 클릭하여 다음 기능을 사용할 수 있습니다:

- **새 프로젝트 생성**: 빈 캔버스로 새 워크플로우 시작
- **프로젝트 불러오기**: 저장된 프로젝트 목록에서 선택하여 불러오기
- **프로젝트 삭제**: 불필요한 프로젝트를 삭제 (확인 다이얼로그 표시)
- **내보내기 (Export)**: JSON 형식으로 워크플로우를 파일로 저장
- **가져오기 (Import)**: JSON 파일에서 워크플로우를 복원
- **저장소 사용량**: LocalStorage 용량 사용 현황 확인

모든 프로젝트는 브라우저 LocalStorage에 자동으로 저장되며, 각 프로젝트는 고유한 ID, 이름, 생성/수정 시간, 버전 번호를 가집니다.

**주의**: LocalStorage는 브라우저별로 독립적입니다. 다른 브라우저에서 접근하려면 Export/Import 기능을 사용하세요.

### 자동 저장 및 단축키

#### 자동 저장

- 노드나 연결선을 변경하면 **2초 후** 자동으로 저장됩니다
- 수동 저장: 상단 툴바의 **저장** 버튼 클릭
- 저장 성공 시 알림 메시지 표시 (Toast notification)

자동 저장은 debounce가 적용되어, 빠르게 여러 변경을 할 때 마지막 변경 후 2초가 지나야 저장됩니다.

#### 키보드 단축키

- **Delete** 또는 **Backspace**: 선택된 노드 삭제
  - 노드를 클릭하여 선택 후 Delete/Backspace 키 입력
  - 노드와 연결된 모든 엣지가 함께 삭제됩니다
- **우클릭**: 노드 컨텍스트 메뉴 열기

### 워크플로우 상태 추적

FlowMatrix는 노드 간 의존성을 자동으로 추적하고 상태를 관리합니다:

#### 노드 상태 종류

| 상태 | 설명 | 시각적 표시 |
|-----|------|----------|
| **PENDING** | 선행 작업을 기다리는 중 | 기본 스타일 |
| **READY** | 실행 가능 (모든 선행 작업 완료) | 밝은 테두리 |
| **IN_PROGRESS** | 현재 진행 중 | 강조 색상 |
| **COMPLETED** | 작업 완료 | 완료 체크 표시 |
| **BLOCKED** | 차단됨 (진행 불가) | 경고 색상 |

#### 자동 상태 업데이트

연결선(엣지)을 추가하면 Workflow Engine이 자동으로 노드 상태를 업데이트합니다:

1. **선행 노드가 완료되면** → 후행 노드가 `READY` 상태로 변경
2. **모든 선행 노드가 완료되지 않으면** → 후행 노드는 `PENDING` 상태 유지
3. **순환 의존성 감지** → 경고 메시지 표시 (미래 버전에서 구현 예정)

#### 수동 상태 변경

노드를 우클릭하여 상태를 수동으로 변경할 수 있습니다. 단, 선행 작업이 완료되지 않은 상태에서 `COMPLETED`로 변경하려고 하면 검증 오류가 발생합니다.

#### 진행률 표시

상단 툴바에 전체 워크플로우의 완료 비율이 표시됩니다:
```
진행률: 3/10 노드 완료 (30%)
```
